        ...
        public TgcMesh toMesh(string meshName)
        {
            ...
            if (Texture != null)
            {
                ...
                var d3dMesh = new Mesh(indices.Count / 3, indices.Count, MeshFlags.Managed,
                    Core.SceneLoader.TgcSceneLoader.DiffuseMapVertexElements, D3DDevice.Instance.Device);
                ...
                using (var vb = d3dMesh.VertexBuffer)
                {
                    ...
                    {
                        var v = new Core.SceneLoader.TgcSceneLoader.DiffuseMapVertex();
            ...
            else
            {
                //Crear Mesh
                var d3dMesh = new Mesh(indices.Count / 3, indices.Count, MeshFlags.Managed,
                    Core.SceneLoader.TgcSceneLoader.VertexColorVertexElements, D3DDevice.Instance.Device);
                ...
                using (var vb = d3dMesh.VertexBuffer)
                {
                    ...
                    for (var j = 0; j < indices.Count; j++)
                    {
                        var v = new Core.SceneLoader.TgcSceneLoader.VertexColorVertex();
                        var vSphere = vertices[indices[j]];
